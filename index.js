...
// ===== IMPROVED TX CONFIRMATION FUNCTION =====
async function waitForConfirmation(provider, txHash, maxWaitSeconds = 45) {
  ...
  throw new Error(`Transaction not confirmed after ${maxWaitSeconds}s`);
}
...
async function sendTransaction(walletData, walletIndex, txNumber, totalTx, totalWallets) {
  ...
  for (let attempt = 1; attempt <= config.batch.max_retries; attempt++) {
    try {
      ...
      spinner.text = `⏳ TX sent, waiting confirmation...`;
      console.log(chalk.gray(`\n   🔗 ${config.network.explorer_url}${tx.hash}`));
      
      // Manual polling with progress
      const startTime = Date.now();
      const maxWaitSeconds = 45;
      let receipt = null;
      
      while (!receipt && ((Date.now() - startTime) / 1000) < maxWaitSeconds) {
        try {
          ...
          spinner.text = `⏳ Polling... ${elapsed}s/${maxWaitSeconds}s (Block pending...)`;
      ...
      if (!receipt || !receipt.blockNumber) {
        throw new Error(`TX not confirmed after ${maxWaitSeconds}s - may still be pending`);
      }
...
async function main() {
  try {
    ...
    if (proxies.length > 0) {
      ...
      console.log(chalk.white(`   Type           : ${chalk.yellow("SOCKS5")}`));
